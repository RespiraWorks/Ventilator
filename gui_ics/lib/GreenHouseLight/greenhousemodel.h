...
public:
    ...
    /// signal, this is mainly used to propagate the model changes via RPC
    ///
    ...
    /// \param silent a flag used to determine if the model is supposed raise the matching notify
    /// signal, this is mainly used to propagate the model changes via RPC \return true if the
    /// update was possible false otherwise
    ///
    ...
    /// filtering, use reset to remove everything
    ///
    void clear();
