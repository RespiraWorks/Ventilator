...
void V03Actuators::init(NVParams::Handler *nv_params) {
  ...
  blower_.emplace(BlowerChannel, BlowerFreq, HalApi::CPUFrequency(), "blower_", " of the blower");

  psol_.emplace(PSolChannel, PSolFreq, HalApi::CPUFrequency(), "psol_",
                " of the proportional solenoid", "position", PSolClosed, PSolOpen);

  buzzer_.emplace(BuzzerChannel, BuzzerFreq, HalApi::CPUFrequency(), "buzzer_", "of the buzzer",
...
void V03System::init() {
  ...
  [[maybe_unused]] bool buffer_size_sufficient = adc_.initialize(HalApi::CPUFrequency());
