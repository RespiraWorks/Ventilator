...
///
/// \brief The Model class is a convenience QAbstractListModel subclass designed to work with JsonData
///
...
public:
    ...
    /// \param silent a flag used to determine if the model is supposed raise the matching notify signal, this is mainly
    ...
    ///
    /// \brief clear used to remove all data from this model, does not remove the custom model filtering, use reset to
    /// remove everything
    ///
    void clear();
...
signals:
    ...
    void delayedPopulateWithRoles(const QStringList &roles, const QJsonArray &data, bool silent) const;
