...
TEST(CommTests, SendControllerStatus) {
  ...
  size_t bufsize = ControllerStatus_size + 4;
  uint8_t decoded_buf[bufsize];
  size_t decoded_length = UnescapeFrame(tx_buffer, tx_length, decoded_buf, bufsize);
  printf("Decoded len: %lu\n", decoded_length);
