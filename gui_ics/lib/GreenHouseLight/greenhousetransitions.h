...
///
/// \brief The TransitionFunctionAPI class defines an interface that lets us add test and trigger functions to
/// transitions
///
...
public:
    ...
    /// \param func
    ///
    /// Test functions are used to determine if a transition should trigger based on the execution context
    ...
    /// \param func
    ///
    /// Trigger functions are executed when a transition is triggered and get access to the execution
...
///
/// \brief The SignalTransition class a custom QSignalTransition subclass designed to work with a Context instance
///
class GREENHOUSESHARED_EXPORT SignalTransition : public QSignalTransition, public TransitionFunctionAPI
{
    Q_OBJECT
...
public:
    ///
    /// \brief SignalTransition creates a new SignalTransition with the given sender, signal and source state
    ...
    /// \param arg
    ///
    /// The parallel triggering setting is used to determine if this transition is allowed to trigger if
    /// the triggering event has already been handled by another transition parented by a parallel state
    /// (default true)
    ///
    ...
    /// \return int
    ///
    /// The state machine level value determines the depth of this transition's source state inside the state machine
    ///
...
///
/// \brief The ItemPropertyChangedTransition class is a SignalTransition subclass designed to trigger on UI element
...
public:
    enum CompareOperator { AnyChange = 0, Equal, NotEqual, GreaterThan, GreaterThanOrEqual, LessThan, LessThanOrEqual };

    ///
    /// \brief ItemPropertyChangedTransition creates a new ItemPropertyChangedTransition instance with the provided
    /// property, compare operator, trigger value and source state
    /// \param property the triggering property's name
    /// \param compareOperator the compare operator to be used when determineing if this transition should trigger
    /// \param triggerValue the value to be used when determineing if this transition should trigger
    /// \param sourceState the transition's source state
    ///
    explicit ItemPropertyChangedTransition(QString property, CompareOperator compareOperator, QVariant triggerValue,
                                           ItemState *sourceState);
...
///
/// \brief The ItemSignalTransition class is a SignalTransition subclass designed to trigger on UI element interface
/// signals
///
...
public:
    ///
    /// \brief ItemSignalTransition creates a new ItemSignalTransition instance with the provided signal name and source
    /// state
