...
class GREENHOUSESHARED_EXPORT Theme : public QObject
{
    ...
    Q_PROPERTY(QString themeBaseDir READ themeBaseDir WRITE setThemeBaseDir NOTIFY themeBaseDirChanged)
...
public:
    ...
    ///
    /// \brief animationFrameAsset used to get an animation frame asset url associated with the provided id
    /// \param id the requested asset's name/id for example "foobar"
    /// \param frame the number of the requested frame
    /// \param frameIdLength the minimum leanght of the frame id generated from the provided frame number
    /// \return
    ///
    Q_INVOKABLE QUrl animationFrameAsset(const QString &id, int frame, int frameIdLength) const;

    ///
    /// \brief coloredAsset used to get an asset url associated with the provided id and the specified color
    /// \param id the requested asset's name/id for example "foobar.png"
    /// \param colorId the desired color's id for the requested asset, can be ignored if no colored match is found
    /// \return
    ///
    ...
    /// \param assetDataStr a string defining the asset's paramteres for example "w_100_h_100_rgb342434"
...
///
/// \brief The ThemeManager class is used to access and manage a Context's currently active theme from QML
///
...
public:
    ...
    ///
    /// \brief contextThemeManagerInstance used to get a ThemeManagerInstance assigned to the provided context
    /// \param context
    /// \return
    ///
    ...
    ///
    static void loadThemeConfig(const QString &configPath, Context *context, const QString &themeBasePath = QString());
    ...
    ///
    /// \brief activateTheme used to set a context's currently active theme based on the provided name
