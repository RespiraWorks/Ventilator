...
class TrendChartWidget : public QQuickPaintedItem
{
    Q_OBJECT
    Q_PROPERTY(
            QAbstractItemModel *dataModel READ dataModel WRITE setDataModel NOTIFY dataModelChanged)
    Q_PROPERTY(QString valueRoleName READ valueRoleName WRITE setValueRoleName NOTIFY
                       valueRoleNameChanged)
    Q_PROPERTY(QString timeStampRoleName READ timeStampRoleName WRITE setTimeStampRoleName NOTIFY
                       timeStampRoleNameChanged)

    Q_PROPERTY(int valueCeiling READ valueCeiling WRITE setValueCeiling NOTIFY valueCeilingChanged)
    Q_PROPERTY(int plotStartIndex READ plotStartIndex WRITE setPlotStartIndex NOTIFY
                       plotStartIndexChanged)
    Q_PROPERTY(int plotPointCount READ plotPointCount WRITE setPlotPointCount NOTIFY
                       plotPointCountChanged)
    Q_PROPERTY(int plotValueOffset READ plotValueOffset WRITE setPlotValueOffset NOTIFY
                       plotValueOffsetChanged)
    Q_PROPERTY(int markedNodeIndex READ markedNodeIndex WRITE setMarkedNodeIndex NOTIFY
                       markedNodeIndexChanged)
    Q_PROPERTY(qreal markerAreaSize READ markerAreaSize WRITE setMarkerAreaSize NOTIFY
                       markerAreaSizeChanged)

    Q_PROPERTY(QColor plotColor READ plotColor WRITE setPlotColor NOTIFY plotColorChanged)
    Q_PROPERTY(qreal plotLineWidth READ plotLineWidth WRITE setPlotLineWidth NOTIFY
                       plotLineWidthChanged)
    Q_PROPERTY(QString nodeMarkerSource READ nodeMarkerSource WRITE setNodeMarkerSource NOTIFY
                       nodeMarkerSourceChanged)

    Q_PROPERTY(
            qreal headerHeight READ headerHeight WRITE setHeaderHeight NOTIFY headerHeightChanged)
    Q_PROPERTY(QColor headerTextColor READ headerTextColor WRITE setHeaderTextColor NOTIFY
                       headerTextColorChanged)

    Q_PROPERTY(QFont headerTextFont READ headerTextFont WRITE setHeaderTextFont NOTIFY
                       headerTextFontChanged)

    Q_PROPERTY(int headerBottomOffset READ headerBottomOffset WRITE setHeaderBottomOffset NOTIFY
                       headerBottomOffsetChanged)

    Q_PROPERTY(bool useTimer READ useTimer WRITE setUseTimer NOTIFY useTimerChanged)
    Q_PROPERTY(
            int timerInterval READ timerInterval WRITE setTimerInterval NOTIFY timerIntervalChanged)

    Q_PROPERTY(bool drawLineToEnd READ drawLineToEnd WRITE setDrawLineToEnd NOTIFY
                       drawLineToEndChanged)
    Q_PROPERTY(int plotLeftMargin READ plotLeftMargin WRITE setPlotLeftMargin NOTIFY
                       plotLeftMarginChanged)
    Q_PROPERTY(int plotRightMargin READ plotRightMargin WRITE setPlotRightMargin NOTIFY
                       plotRightMarginChanged)
