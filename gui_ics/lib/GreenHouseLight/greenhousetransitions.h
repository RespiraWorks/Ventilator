...
public:
    ...
    /// context and the provided aregument list extracted from an event
    ///
    ...
    /// execution context as well as the argument list extracted from the triggering event
    ///
...
public:
    ...
    /// source state \param sender a pointer to a sender object \param signal the name of the
    /// triggering signal \param sourceState the transition's source state
    ///
    ...
    /// trigger if the triggering event has already been handled by another transition parented by a
    /// parallel state (default true)
    ///
...
/// trigger on UI element interface property changes
///
...
public:
    ...
    /// with the provided property, compare operator, trigger value and source state \param property
    /// the triggering property's name \param compareOperator the compare operator to be used when
    /// determineing if this transition should trigger \param triggerValue the value to be used when
    /// determineing if this transition should trigger \param sourceState the transition's source
    /// state
    ///
...
public:
    ...
    /// signal name and source state \param signalName the triggering signal's name \param
    /// sourceState the transition's source state
    ///
