...
TEST(SpiDaisyChain, RequestQueue) {
  ...
  EnqueueRequestAndTestResult(&daisy_chain, extra_request, NumSlaves + 1,
  ...
  // Dequeueing and checking that all requests have been procesed, as well as the resulting spi
  // flow.
...
#define QUEUE_NEXT_REQUEST(target)                    \
  ...
  request = {.command = command[next_command],        \
...
TEST(SpiDaisyChain, SendDuringTransmission) {
  ...
  while (!processed[4]) {
    ...
    for (size_t slave = 0; slave < NumSlaves; slave++) {
      expected[slave] =
          transmitting[slave] ? command[current_request[slave]][next_byte[slave]++] : NullCommand;
